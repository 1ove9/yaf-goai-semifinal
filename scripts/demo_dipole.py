...
async def run_demo() -> None:
    ...
    # 2. Create geometry: a single wire along z — NEC2 is a wire-grid
    # solver, so the dipole is one 2-node edge, not a surface mesh
    length = wavelength / 2
    geom = Geometry(
    ...
    print(f"  Solver mode: {result.solver_metadata.get('solver_mode', 'unknown')}")
    print(f"  Simulation time: {result.simulation_time_sec:.2f} s")

    if result.solver_metadata.get("solver_mode") == "fallback_analytical":
        print("\n  " + "!" * 58)
        print("  !! ANALYTICAL FALLBACK — this is NOT an EM simulation.")
        print("  !! nec2c was not found; values come from the induced-EMF")
        print("  !! closed-form dipole model (Balanis eq. 8-60/8-61).")
        print("  !! Install nec2c for real MoM results (docs/next-steps.md).")
        print("  " + "!" * 58)
