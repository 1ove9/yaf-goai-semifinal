...
class ParametricGenerator:
    ...
    def rectangular_patch(
        ...
        *,
        ...
        feed_x: float | None = None,
        substrate_dispersion: dict[str, Any] | None = None,
    ) -> Geometry:
        ...
        Args:
            ...
            substrate_width: Substrate width [m] (along y); patch width if None.
            substrate_length: Substrate length [m] (along x); patch length if None.
            eps_r: Substrate relative permittivity (default: FR-4).
            loss_tangent: Substrate loss tangent (default: FR-4).
            feed_x: Probe-feed x offset from patch center [m];
                -3/16 of the resonant length if None (offset ratio of the
                openEMS tutorial patch, a good 50 Ω starting point).
            substrate_dispersion: Optional Drude/Lorentz pole on top of
                eps_r, e.g. {"model": "lorentz", "plasma_freq_hz": ...,
                "pole_freq_hz": ..., "relax_time_s": ...} (Hz / s).

        Returns:
            Geometry with patch, substrate, and optional ground plane.
            Solver-relevant parameters (substrate size/material, feed
            position) are carried in ``metadata`` for adapters that
            rebuild the structure parametrically (e.g. openEMS FDTD).
        """
        vertices: list[list[float]] = []
        faces: list[list[int]] = []
        ...
        return Geometry(
            ...
            metadata={
                ...
                "substrate_width": substrate_width if substrate_width is not None else width,
                "substrate_length": substrate_length if substrate_length is not None else length,
                "eps_r": eps_r,
                "loss_tangent": loss_tangent,
                "feed_x": feed_x if feed_x is not None else -length * 3.0 / 16.0,
                **(
                    {"substrate_dispersion": substrate_dispersion}
                    if substrate_dispersion is not None else {}
                ),
            },
        )
