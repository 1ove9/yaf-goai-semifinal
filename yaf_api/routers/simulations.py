...
class SimulationRequest(BaseModel):
    """Request body for submitting a simulation job."""

    design_id: uuid.UUID
    solver: str = "nec2"
    frequency_min: float = Field(default=1e9, gt=0)
    frequency_max: float = Field(default=2e9, gt=0)


@router.post("", response_model=SimulationJob)
async def submit_simulation(req: SimulationRequest) -> SimulationJob:
    ...
    spec = SimulationSpec(
        name="simulation",
        frequency_range=(req.frequency_min, req.frequency_max),
        frequency_points=51,
    )
